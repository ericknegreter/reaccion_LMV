        ...
        if humidity is not None and temperature is not None:
            if temperature > 21.5 and con == 1:
                ...
                if now.hour >= 8 and now.hour < 18:
                    ...
                    while True:
                        if(net_is_up() == 0):
                            #Connection to database LMV and update record in e_reaccion table with mysql
                            mydb = mysql.connector.connect(host="10.0.5.246", user="LMV_ADMIN", passwd="LABORATORIOT4", database="LMV")
                            mycursor = mydb.cursor()
                            sql = "UPDATE e_reaccion SET estado = 1 WHERE dispositivo='aire'"
                ...
                if now.hour >= 8 and now.hour < 18:
                    ...
                    while True:
                        if(net_is_up() == 0):
                            ...
                            mydb = mysql.connector.connect(host="10.0.5.246", user="LMV_ADMIN", passwd="LABORATORIOT4", database="LMV")
                            mycursor = mydb.cursor()
                            sql = "UPDATE e_reaccion SET estado = 0 WHERE dispositivo='aire'"
                            mycursor.execute(sql)
                            mydb.commit()
                            print(mycursor.rowcount, "record affected.")
                            time.sleep(1)
